...
class BossBarViewModel extends AbstractViewModel<Station, BossBar, UUID> {
    ...
    IFactory<BossBar, Station> getViewFactory() {
        return data -> {
            BossBar bossBar = BossBar.bossBar(Component.empty(), 0, DEFAULT_BOSS_BAR_COLOR, Overlay.PROGRESS);
            if (data == null) {
                return bossBar;
            }
    ...
    private static void updateBossBar(@NotNull BossBar reference, Station station) {
        ...
        final Component formattedBossBarName = MiniMessage.miniMessage().deserialize("Forging: <a> (<b>)",
                Placeholder.component("a", retrieveItemName(recipe)), Placeholder.component("b", Component.text(actions - progress)));
